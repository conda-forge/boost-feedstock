...
# B2 is a build-platform tool that links the SDK libc++; keep its libc++ 21 hash fallback enabled.
CXXFLAGS="$(echo ${CXXFLAGS} | sed 's/ -D_LIBCPP_DISABLE_AVAILABILITY//g' | sed 's/ -march=[^ ]*//g' | sed 's/ -mcpu=[^ ]*//g' |sed 's/ -mtune=[^ ]*//g')" \
